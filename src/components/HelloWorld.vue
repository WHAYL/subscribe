                        ...
                        <el-collapse-item :title="indexs+'楼'" :name="indexs+'楼'" v-if="indexs!=0 && items!=undefined">
                            ...
                            <div style="display: flex;justify-content:space-around" v-for="(itemss,indexss) in items" :key="indexss">
                                ...
                                <div style="margin-top: 15px">
                                    ...
                                    <el-button type="danger" round @click="deleteapartment(item[0],itemss[0],itemss[1])">删除</el-button>
                                </div>
                            </div>
                        </el-collapse-item>
                    </el-collapse>
        ...
        methods: {
            load(){
                ...
                for(var i=0;i<this.data.length;i++){
                    ...
                    if(building.indexOf(this.data[i].building.building)==-1){
                        ...
                        this.apartment[j][this.data[i].laboratory[0]][x]=[];
                        ...
                        if(this.data[i].laboratory[0]!=1){
                            if(!Array.isArray( this.apartment[j-1][this.data[i].laboratory[0]]))
                            {    x=0;
                                this.apartment[j-1][this.data[i].laboratory[0]]=[];
                                this.apartment[j-1][this.data[i].laboratory[0]][x]=[];

                            }
                        }
                        this.apartment[j-1][this.data[i].laboratory[0]][x]=[];
                        this.apartment[j-1][this.data[i].laboratory[0]][x].push(this.data[i].id);
                        this.apartment[j-1][this.data[i].laboratory[0]][x].push(this.data[i].laboratory);
                        this.apartment[j-1][this.data[i].laboratory[0]][x].push(this.data[i].status);
                        x++;
            ...
            deleteapartment(a,b,c){
                ...
                this.$confirm('此操作将永久删除该实验室, 是否继续?', '提示', {
                    ...
                    this.$http.post(this.$apis.deletelaboratory,data).then((resp)=>{
                        if(resp.data){
