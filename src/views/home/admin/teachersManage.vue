        ...
        <div id="zz" v-if="show">
            <el-form :model="addform" :rules="rules" ref="addruleForm" label-width="100px" class="demo-ruleForm">
                <el-form-item label="教师账号" prop="username">
                    <el-input v-model="us" style="width: 300px" disabled="true" placeholder="请输入账号" ></el-input>
                </el-form-item>
                <el-form-item label="教师密码" prop="password">
                    <el-input v-model="us" style="width: 300px" disabled="true" show-password placeholder="请输入密码" ></el-input>
                </el-form-item>
                <el-form-item label="姓名" prop="name">
    ...
    export default {
        ...
        methods: {
                ...
                this.$refs.ruleForm.validate((valid) => {
                    if (valid) {
                        ...
                        this.$confirm('确定修改吗？')
                            .then(async _ => {
                                this.loading = true;
                                await this.$http.post(this.$apis.updateteacher,this.form).then(async (resp)=>{

                                    if(resp.data){
                                      await  this.$http.get(this.$apis.teachers).then((resp)=>{
            ...
            handleDelete(index, row) {
                this.$confirm(`此操作将删除     教师     ${row.name}, 是否继续?`, '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(async () => {
                    await this.$http.post(this.$apis.deleteteacher,{t_id:row.id,u_id:row.users_id}).then((resp)=>{
                        if(resp.data){
                            this.$http.get(this.$apis.teachers).then((resp)=>{
                                this.tableData=resp.data;
                            })
                            this.$message({
                                type: 'success',
            ...
            addteacher(){
                this.$refs.addruleForm.validate((valid) => {
                    if (valid) {
                        this.addform.username= this.us;
                        this.addform.password= SparkMD5.hash( this.us);

                        this.$http.post(this.$apis.addteacher,this.addform).then(async (resp)=>{
                            if(resp.data){
                               await this.$http.get(this.$apis.teachers).then((resp)=>{
                                    this.tableData=resp.data;
                                });
